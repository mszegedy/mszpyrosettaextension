...
def pose_from_file_with_params(filename, params_filenames):
    ...
    '''
    pose = pr.Pose()
    pr.generate_nonstandard_residue_set(pose, params_filenames)
    pr.pose_from_file(pose, filename)
    return pose

def pose_from_pdbstring_with_params(pdbstring, params_filenames):
...
def res_neighbors_p(pose, resnum1, resnum2, coarsep=False, bound=None):
    ...
    are two possible standards that the algorithm employs:
        coarsep=False: any two non-hydrogen atoms in the two residues'
          sections of protein are 4.5 Ao or less apart
